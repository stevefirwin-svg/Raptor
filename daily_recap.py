...
def get_portfolio_analytics(closed_trades, equity):
    ...
    # P0-7 fix: Sharpe/Sortino annualization must use avg hold period, not 252.
    # sqrt(252) assumes daily returns; Raptor trades are multi-day swings so the
    # correct annualization factor is sqrt(252 / avg_hold_days).
    hold_days_list = []
    for t in closed_trades:
        hd = t.get("hold_days")
        if hd is not None and hd > 0:
            ...
            try:
                ...
                _ed = _dt.strptime(str(t["entry_date"])[:10], "%Y-%m-%d")
                _xd = _dt.strptime(str(t["exit_date"])[:10], "%Y-%m-%d")
                _hd = (_xd - _ed).days
                if _hd > 0:
                    hold_days_list.append(_hd)
            except Exception:
                pass
    avg_hold = max(1.0, float(np.mean(hold_days_list))) if hold_days_list else 15.0
    annualization_factor = np.sqrt(252.0 / avg_hold)
    ...
    sortino = (mean_r / downside_std) * annualization_factor if downside_std > 0 else 0.0
...
def get_capital_utilization(positions, equity):
    ...
    if not equity or equity == 0:
        return 0.0
    total_mv = sum(abs(float(p.get("qty", 0))) * abs(float(p.get("current_price", 0))) for p in positions)
...
def get_position_beta(positions, dm):
    ...
    try:
        ...
        for p in positions:
            sym = p["symbol"]
            mv = abs(float(p.get("qty", 0))) * abs(float(p.get("current_price", 0)))
...
def build_html(account, positions, entries, exits, signals, macro, scale,
    ...
    # Pre-compute health HTML outside f-string to catch errors cleanly
    try:
        health_html_block = build_health_html()
    except Exception as e:
        logger.warning(f"build_health_html failed: {e}")
        health_html_block = '<div style="color:#a0a0b0">Health monitor data unavailable</div>'
...
def send_email(html, positions):
    ...
    log_path = f"logs/recap_{date_str}.log"

    try:
        with smtplib.SMTP_SSL("smtp.gmail.com", 465) as s:
            s.login(EMAIL_SENDER, EMAIL_PASSWORD)
            s.send_message(msg)
        msg_ok = f"[{datetime.now().strftime('%H:%M:%S')}] Recap email sent OK."
        print(msg_ok)
        with open(log_path, "a") as lf:
            lf.write(msg_ok + "\n")
    except Exception as e:
        msg_err = f"[{datetime.now().strftime('%H:%M:%S')}] Email FAILED: {e}"
        print(msg_err)
        with open(log_path, "a") as lf:
            lf.write(msg_err + "\n")
...
def main(preview=False):
    ...
    try:
        ...
        import traceback
        err = f"[{datetime.now().strftime('%H:%M:%S')}] daily_recap CRASHED: {e}\n{traceback.format_exc()}"
        print(err)
        with open(log_path, "a") as lf:
            lf.write(err + "\n")
